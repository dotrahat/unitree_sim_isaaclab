...
class ObjectTableSceneCfg(TableCylinderSceneCfgWH):
    ...
    # z=0.8: matches the crouch stance's pelvis->sole distance (0.7846 m, measured via FK on the
    # URDF at hip_pitch=-0.1/knee=0.3/ankle_pitch=-0.2), leaving ~1.5 cm clearance above the floor.
    # z=0.755 (the previous value) buried the soles ~3 cm below the floor and caused the robot to
    # tip/fall at spawn instead of settling. Matches the other move_cylinder_g1_29dof_*_wholebody
    # tasks, which all spawn at z=0.8 in this same scene.
    robot: ArticulationCfg = G1RobotPresets.g1_23dof_brainco_wholebody(init_pos=(-3.9, -2.81811, 0.8),
        init_rot=(1, 0, 0, 0))
...
@configclass
class EventCfg:
    # Isaac Lab's Articulation.reset() does NOT apply init_state.joint_pos (it only
    # resets actuators) -- without this event the robot starts with the USD's zero
    # (straight-leg) pose, feet penetrating the floor, and tumbles violently at
    # policy start. The 29dof wholebody tasks have an empty EventCfg and inherit
    # that startup transient; policy-23dof.onnx does not survive it, and its
    # observation depends on starting at its own trained default pose (including
    # the arm defaults, which the policy observes even though it doesn't control
    # them -- see action_provider_wh_g123_dds.py).
    reset_all = EventTermCfg(func=mdp.reset_scene_to_default, mode="reset")


@configclass
class MoveCylinderG123BraincoWholebodyEnvCfg(ManagerBasedRLEnvCfg):
    """
